...
하지만 이 공간이동 장치는 이동 거리를 급격하게 늘릴 경우 기계에 심각한 결함이 발생하는 
단점이 있어서, 이전 작동시기에 k광년을 이동하였을 때는 k-1 , k 혹은 k+1 광년만을 
다시 이동할 수 있다. 예를 들어, 이 장치를 처음 작동시킬 경우 -1 , 0 , 1 
광년을 이론상 이동할 수 있으나 사실상 음수 혹은 0 거리만큼의 이동은 의미가 없으므로 
1 광년을 이동할 수 있으며, 
...
하지만 y지점에 도착해서도 공간 이동장치의 안전성을 위하여 y지점에 도착하기 바로 직전의 
...
class Main {
    public static void main(String[] args) throws IOException {
        ...
        while(read < N){
            ...
            if(sqrtDist == (int)sqrtDist)ans = (int) (sqrtDist*2-1);
            else{
                double a = Math.pow(Math.ceil(Math.sqrt(y-x)), 2); 
                double b = Math.pow(Math.ceil(Math.sqrt(y-x)) - 1, 2) + 1; 
                if((a+b) / 2 <= y-x){
                    ans = (int) (2 * Math.ceil(Math.sqrt(y-x)) - 1);
                }else{
                    ans = (int) (2 * Math.ceil(Math.sqrt(y-x)) - 2);
                }
            }
            sb.append(ans+"\n");
            read++;
