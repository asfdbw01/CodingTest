...
class Main {
    ...
    private static void run(BufferedReader br,int N) throws IOException{
        int read = 0;

        while (read < N) {
            StringTokenizer st = new StringTokenizer(br.readLine());
            String str =  st.nextToken();
            if (read==0)initPattern(str);
            comparePattern(str);
            read++;
        }
    ...
    private static void comparePattern(String str){
        int len = str.length();
        for(int i =0;i<len;i++){
            if(pattern[i]==str.charAt(i))continue;
            else pattern[i] = '?';
        }
    }

    private static  boolean isAlphabetOrPattern(char c){
        return (c>='a'  && c <='z') || (c>='A' && c<='Z') || c=='.';
    }


    private static void initPattern(String str){
        int len = str.length();
        pattern = new char[len];
        for(int i=0;i<len;i++){
            char c= str.charAt(i);
            if(isAlphabetOrPattern(c))pattern[i] =c;
            else pattern[i] ='?';
        }
    }
}